...
public class VinControllerIntegrationTest {
	...
	// le mapper permet de convertir nos données en JSON lorsque nous voulons
	// invoquer notre API
	private ObjectMapper mapper = new ObjectMapper();

	@Test
	public void DoitRecupererTousLesVins() throws Exception {
		mvc.perform(get("/vin")).andExpect(status().isOk()).andExpect(jsonPath("$.length()", is(4)))
				.andExpect(jsonPath("$.[0].chateau", is("Château Margaux")));
	}

	@Test
	public void DoitRecupererUnVin() throws Exception {
		mvc.perform(get("/vin/1")).andExpect(status().isOk()).andExpect(jsonPath("$.id", is(1)))
				.andExpect(jsonPath("$.chateau", is("Château Margaux")));
	}

	@Test
	public void DoitRecupererUnVinInexistant() throws Exception {
		mvc.perform(get("/vin/50")).andExpect(status().isNotFound());
	}

	@Test
	public void DoitRecupererUnVinInvalide() throws Exception {
		mvc.perform(get("/vin/0")).andExpect(status().isBadRequest());
	}
	...
	public void DoitAjouterVin() throws Exception {
		...
		mvc.perform(post("/vin").contentType(MediaType.APPLICATION_JSON).content(mapper.writeValueAsString(vin1)))
				.andExpect(status().isCreated());

		mvc.perform(get("/vin")).andExpect(status().isOk()).andExpect(jsonPath("$.length()", is(5)))
				.andExpect(jsonPath("$.[1].chateau", is("Château Cantemerle")))
				.andExpect(jsonPath("$.[1].prix", is(30.5))).andExpect(jsonPath("$.[4].chateau", is("Château 1")))
				.andExpect(jsonPath("$.[4].appellation", is("Saint-Julien"))).andExpect(jsonPath("$.[4].id", is(5)));
	}
	...
	public void DoitMettreAJourVin() throws Exception {
		...
		mvc.perform(put("/vin").contentType(MediaType.APPLICATION_JSON).content(mapper.writeValueAsString(vin1)))
				.andExpect(status().isOk());

		mvc.perform(get("/vin/1")).andExpect(status().isOk()).andExpect(jsonPath("$.chateau", is("Château 1")))
