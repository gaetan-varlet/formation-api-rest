...
public class TestControllerIntegrationTest {
	...
	public void DoitRecupererNomPasAuthentifie() throws Exception {
		mvc.perform(get("/mon-nom")).andExpect(status().isUnauthorized());
	}

	@Test
	@WithMockUser
	public void DoitRecupererNom() throws Exception {
		mvc.perform(get("/mon-nom")).andExpect(status().isOk()).andExpect(content().string("Gaetan"));
		;
	}
	...
	public void DoitRecupererEnvironnement() throws Exception {
		mvc.perform(get("/environnement")).andExpect(status().isOk())
				.andExpect(jsonPath("$", is("environnement local")));
	}

	@Test
	public void DoitRecupererEnvironnementPasAuthentifie() throws Exception {
		mvc.perform(get("/environnement")).andExpect(status().isUnauthorized());
	}

	@Test
	@WithMockUser(roles = "MAUVAIS_ROLE")
	public void DoitRecupererEnvironnementRoleInvalide() throws Exception {
		mvc.perform(get("/environnement")).andExpect(status().isForbidden());
	}

	@Test
	public void uploadFichier() throws Exception {
		MockMultipartFile firstFile = new MockMultipartFile("multipartfile", "filename.txt", "text/plain",
				"coucou".getBytes());

		mvc.perform(multipart("/upload").file(firstFile)).andExpect(status().isOk())
	...
	public void uploadFichier2() throws Exception {
		...
		mvc.perform(multipart("/upload").file(multipartFile)).andExpect(status().isOk())
				.andExpect(content().string("Je m'appelle Toto"));
	}

	@Test
	public void DoitRetournerIdepSansIdep() throws Exception {
		mvc.perform(get("/principal")).andExpect(status().isOk()).andExpect(jsonPath("$", is("Mon idep est null.")));
	}

	@Test
	@WithMockUser(username = "toto")
	public void DoitRetournerIdepAvecIdep() throws Exception {
		mvc.perform(get("/principal")).andExpect(status().isOk()).andExpect(jsonPath("$", is("Mon idep est toto.")));
	}

	@Test
	@WithMockUser(roles = "toto")
	public void DoitValiderRole() throws Exception {
		mvc.perform(get("/role/toto")).andExpect(status().isOk()).andExpect(jsonPath("$", is(true)));
	}

	@Test
	public void DoitInvaliderRole() throws Exception {
		mvc.perform(get("/role/toto")).andExpect(status().isOk()).andExpect(jsonPath("$", is(false)));
	}
